...
without collapsing to background. In a held-out continent experiment (Asia and Oceania
withheld from training) accuracy does not collapse on the unseen regions, although that
partition also has a more favourable class composition, which we quantify. We further use
area-stratified recall to localize where detection fails: recall falls to \num{0.44} on
small ($<32^2$\,px) sites, the operationally decisive regime for artisanal mining. At
\num{1.06}\,M parameters and \textless\,\num{0.65}\,GFLOPs, SPEAR-Net is inexpensive
enough to screen large areas on free-tier hardware. We frame the result as parity with
much larger baselines at a fraction of the compute, together with a diagnosis of the
small-site limitation, rather than as an accuracy improvement.
\end{abstract}
...
a \emph{held-out continent} generalization protocol that is rarely reported for mining
...
Asia and Oceania, which is what later enables the held-out continent protocol of
...
five continents and underpins the held-out continent generalization protocol.}
\label{fig:map}
\end{figure}
...
indicating that both scales are recovered rather than one dominating. SPEAR-Net thus
reaches within \textasciitilde\,\num{0.013} mIoU of the best baseline at \num{1.06}\,M
parameters and $\leq$\,\num{0.61} GFLOPs; we did not train an efficiency-oriented baseline,
so we report this as a compute profile (Section~\ref{sec:results-gen}) rather than as a
comparison against other compact models. This trade-off is visualized in
Fig.~\ref{fig:efficiency}: the compact variants form a cluster within \num{0.02} mIoU of
the baselines while sitting more than an order of magnitude to the left on the
...
Table~\ref{tab:ablation} isolates each component of SPEAR-Net. All ablation rows use
identical chips, splits, random seed and training schedule; the RGB-prior variant uses the
same six-band chips restricted to the visible (B, G, R) bands, so its comparison against
the six-band prior is controlled. Adding the recall-weighted objective to the plain
backbone raises recall from
...
Table~\ref{tab:gen} reports the held-out continent experiment in which Asia and Oceania
are withheld from training. SPEAR-Net scores \num{0.734} mIoU on the held-out continents
versus \num{0.618} in-region. We do not read the higher out-of-region score as evidence of
positive transfer: the two partitions are not comparable, because holding out Asia and
Oceania also changes the training-set size and the class/object-size composition of each
partition (Section~\ref{sec:data}; the in-region \num{0.618} is therefore not comparable to
the \num{0.639} of Table~\ref{tab:main}, which was trained on all regions). The defensible
reading is narrow: accuracy does not \emph{collapse} on the two unseen continents, so no
catastrophic domain shift is observed in this single hold-out. A full rotation over regions,
with matched composition and baselines evaluated under the same protocol, is left to future
work.
...
concatenation: on mIoU they differ by less than \num{0.01}, and we did not measure
run-to-run variance, so we treat them as comparable rather than ordered. The gate's
...
held-out continent rotation, and evaluation of the baselines under the same protocol,
...
Trained on a compact, expert-verified dataset with an explicit scale label, SPEAR-Net
matches strong convolutional baselines to within about \num{1.3} mIoU points and at
comparable recall while using \num{20}--\num{25}$\times$ fewer parameters and roughly
\num{26}--\num{70}$\times$ fewer FLOPs, and recovers both mining scales without background
collapse; its explainable spectral-prior attention makes each detection interpretable in
terms of named remote-sensing indices. Equally important is the diagnosis: an
area-stratified analysis shows that small-site recall (\num{0.44}) is the binding
limitation, and that it is \emph{not} closed by baselines with \num{20}--\num{25}$\times$
more parameters, indicating a structural rather than a capacity constraint. Because it is
small, fast and free-tier trainable, SPEAR-Net is suited to large-area \emph{screening and
prioritisation} rather than enforcement-grade detection; future work will target the
small-site regime, multi-seed and full region-rotation evaluation, and transfer to
auxiliary ASM domains.
