...
interpretation of optical imagery \citep{maus2022global} and, more recently, from deep
learning applied to Sentinel data \citep{tang2023mining}. For ASM specifically,
\citet{gallwey2020asm} trained a multispectral convolutional network to detect
small-scale mining in Ghana, and \citet{balaniuk2020mining} addressed mining and tailings
detection with deep networks. A consistent theme is that fragmented, low-density mining is
hard to recover and easily confused with spectrally similar background, motivating methods
that inject physical priors and explicitly optimize recall.
...
Attention U-Net \citep{oktay2018attentionunet}, remain strong baselines for dense
prediction, as do atrous encoder--decoders such as DeepLabV3+
\citep{chen2018deeplabv3plus} and transformer segmenters such as SegFormer
\citep{xie2021segformer}. These models are accurate but parameter-heavy (tens of millions
of weights). Efficiency-oriented backbones---MobileNetV3 \citep{howard2019mobilenetv3},
EfficientNet \citep{tan2019efficientnet} and depthwise-separable convolutions
...
\citep{salehi2017tversky}), the focal loss \citep{lin2017focal} and boundary-aware losses
\citep{kervadec2019boundary} were introduced to counter class imbalance and to sharpen
...
scale attribute reliable enough to serve as a training target.
...
\begin{figure}[t]
\centering
% ---- FIGURE PLACEHOLDER: replace with the architecture diagram ----
\fbox{\begin{minipage}[c][0.30\textheight][c]{0.92\linewidth}\centering
\textit{[Figure~\ref{fig:arch}: SPEAR-Net architecture. Six-band Sentinel-2 input
$\rightarrow$ PISP index computation and gate $\rightarrow$ MobileNetV3-Small encoder
(stem adapted to six bands) $\rightarrow$ depthwise-separable U-Net decoder with
prior-gated skips $\rightarrow$ segmentation head + edge head. Insert vector figure
\texttt{figures/architecture.pdf}.]}
\end{minipage}}
\caption{SPEAR-Net architecture. The four spectral indices are turned into a single spatial
attention map that gates each decoder skip connection; an auxiliary edge head provides deep
supervision.}
\label{fig:arch}
\end{figure}
...
\label{eq:tversky}
\end{equation}
A class-weighted focal term \citep{lin2017focal} supplies a per-pixel gradient that focuses
...
baselines use the implementations of \citet{iakubovskii2019smp} with the same six-band
input, loss and schedule for a controlled comparison. Training used a single NVIDIA A100
GPU; the model also fits and trains within the memory of a free-tier 16\,GB accelerator.

\subsection{Evaluation}\label{sec:eval}
We report per-class Intersection-over-Union
$\mathrm{IoU}_c = \mathrm{TP}_c/(\mathrm{TP}_c+\mathrm{FP}_c+\mathrm{FN}_c)$, the mean IoU
over present classes, per-class recall $\mathrm{TP}_c/(\mathrm{TP}_c+\mathrm{FN}_c)$, and
overall recall. To probe the field-wide small-object failure directly, we compute an
\emph{area-stratified recall}: for each foreground class we extract connected components of
the ground truth, bin them by pixel area into small ($<32^2$), medium and large
($\geq96^2$), and count a component as recovered if any of its pixels is predicted as that
class. Efficiency is reported as parameter count and multiply--accumulate operations
(GFLOPs) at $256^2$ input. For geographic generalization we assign each chip to a continent
from its centroid and perform a leave-one-region-out experiment, training with two
continents held out and testing on them.
...
favourable operating point for large-area, resource-constrained inference.
...
the gate as a recall-oriented design choice rather than an accuracy improvement.
...
from \num{0.44} for small components to \num{0.92} for large ones. The small-object regime
is therefore the dominant remaining error source, which is consistent with the difficulty
of fragmented artisanal sites and delineates a concrete target for future work.
...
\begin{figure}[t]
\centering
% ---- FIGURE PLACEHOLDER: qualitative predictions + attention ----
\fbox{\begin{minipage}[c][0.26\textheight][c]{0.92\linewidth}\centering
\textit{[Figure~\ref{fig:qual}: For several validation chips, columns show (a) Sentinel-2
true-colour, (b) ground truth, (c) SPEAR-Net prediction, (d) PISP attention overlay.
Insert \texttt{figures/qualitative.pdf}.]}
\end{minipage}}
...
\begin{figure}[t]
\centering
% ---- FIGURE PLACEHOLDER: PISP index maps ----
\fbox{\begin{minipage}[c][0.20\textheight][c]{0.92\linewidth}\centering
\textit{[Figure~\ref{fig:priors}: Sentinel-2 true colour, ground-truth mask, and the four
PISP index maps (NDVI, MNDWI, NDTI, BSI) for a representative scene. Insert
\texttt{figures/priors.pdf}.]}
\end{minipage}}
